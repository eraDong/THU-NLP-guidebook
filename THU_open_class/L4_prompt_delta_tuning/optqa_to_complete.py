...
parser.add_argument("--data_path", type=str, default = "./ARC-DA-v1.1")
parser.add_argument("--plm_path", type=str, default = "./opt-125m")
...
def get_dataset(args):
    ...
    #############################################################
    # 创建训练集、验证集和测试集
    origin_datasets = load_dataset("json", data_files={"train": args.data_path + "/train.jsonl",
                                               "dev": args.data_path + "/dev.jsonl",
                                               "test": args.data_path + "/test.jsonl"})
    ...
    for key in origin_datasets.keys():
        for item in origin_datasets[key]:
            item["answers"] = item["answers"][0]
            inputexample = InputExample(
                guid = item['question_id'],
                text_a = item['question'],
                text_b = item['answers'],
                meta = { "answer":item["answers"] },
                label=0
            )
            datasets[key].append(inputexample)
...
def add_delta_to_backbone(plm,args):
    ...
    config = AutoDeltaConfig.from_dict({"delta_type":args.delta_type})
    delta_model = AutoDeltaModel.from_config(config, plm)
    # delta_model.attach()
...
def load_from_finetuned(plm, args):
    ...
    ##
    #############################################################
    delta_save_path = args.delta_save_path
    delta_model = AutoDeltaModel.from_finetuned(delta_save_path, backbone_model=plm)
    delta_model.eval()
...
def get_prompt_template(tokenizer, args):
    ...
    mytemplate = ManualTemplate(
        text = '{"placeholder":"text_a"}. In this question, answer is {"mask"}',
        tokenizer = tokenizer
    )
    return mytemplate
...
def main(args):
    ...
    ##
    #####################################################################
    optimizer = torch.optim.AdamW(plm.parameters(), lr=args.lr)
    ...
    for epoch in range(args.train_epochs):
        ...
        for step, inputs in enumerate(pbar):
            ...
            optimizer.zero_grad()
            inputs = inputs.to(device)
            # outputs,_ = prompt_model.generate(inputs)
            # loss = prompt_model.loss_fct(outputs,)
            loss = prompt_model.forward(inputs)
            loss.backward()
            optimizer.step()
            scheduler.step()
            tot_loss += loss.item()
        ...
        print(f"ValAcc {val_f1},Tot loss {tot_loss}")
